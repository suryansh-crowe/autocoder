...
autocoder/                        Project root (= the "autocoder" project)
  ...
  pytest.ini                      Markers, testpaths (feature files resolve next to the test module)
  ...
  src/                            Python source (src layout)
    autocoder/                    The installable package, imported as `autocoder`
      ...
      generate/                   Stages 5 + 7: deterministic renderers
        pom.py                      Render tests/generated/<run>/<slug>/<slug>_page.py
        feature.py                  Render tests/generated/<run>/<slug>/<slug>.feature
        steps.py                    Render tests/generated/<run>/<slug>/test_<slug>.py
  ...
  tests/                          The Playwright BDD suite (generated + scaffold)
    ...
    generated/                      Per-run bundles — one folder per `autocoder generate` invocation
      __init__.py                   Package marker
      conftest.py                   Collection filter: keep only the newest run folder per slug
      generated_<YYYYMMDD_HHMMSS>/  One per run; contains the slug bundles + manifest snapshot
        <slug>/
          <slug>.feature              Gherkin
          test_<slug>.py              pytest-bdd step module
          <slug>_page.py              Generated POM
          results.xml                 JUnit xml from `autocoder report --run` / heal
        manifest/                     This run's manifest root (the ONLY manifest location)
          registry.yaml                 URL registry at end-of-run
          extractions/<slug>.json       Per-URL extractions + .prev.json diffs
          plans/<slug>_*.json           Cached POM + feature plans
          heals/<slug>_*.json           Cached heal suggestions (stub + failure)
          logs/<ts>-<cmd>.log           Per-invocation run log
          runs/                         Legacy session-xml landing pad (only used when
                                         the flat-layout fallback kicks in)
    pages/
  ...
  (no root-level manifest/ directory — every run folder carries its own)

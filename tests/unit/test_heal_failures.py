...
* JUnit XML parsing extracts test_id, file path, error message,
  the failing ``test_*`` function name (deepest in the traceback,
  falling back to the testcase name), and the failure classifier
  label.
...
<testcase classname="tests.playwright.test_login" name="test_smoke_login" file="tests/playwright/test_login.py">
<failure message="Locator.click: Timeout 30000ms exceeded." type="playwright._impl._errors.TimeoutError">
tests/playwright/test_login.py:28: in test_smoke_login
    pom.click_sign_in_with_microsoft()
...
<testcase classname="tests.playwright.test_login" name="test_validation_email" file="tests/playwright/test_login.py">
<failure message='Input of type "checkbox" cannot be filled' type="playwright._impl._errors.Error">
tests/playwright/test_login.py:48: in test_validation_email
    pom.fill_email('your-email@example.com')
...
def test_parse_junit_xml_extracts_failures(tmp_path: Path) -> None:
    ...
    assert f1.test_function == "test_smoke_login"
    assert f1.failure_class == "disabled"
    assert "Timeout" in f1.error_message

    assert f2.test_function == "test_validation_email"
...
def test_parse_junit_xml_skips_passing_tests(tmp_path: Path) -> None:
    ...
    p.write_text(
        ...
        '<testcase classname="tests.playwright.test_x" name="test_pass"/>'
...
def test_parse_junit_xml_handles_error_elements(tmp_path: Path) -> None:
    ...
    p.write_text(
        ...
        'classname="tests.playwright.test_login" name="test_x">'
        '<error message="boom" type="ImportError">tests/playwright/test_login.py:1: ImportError</error>'
...
def test_parse_junit_xml_falls_back_to_testcase_name(tmp_path: Path) -> None:
    """When the traceback mentions no ``test_*`` frame, fall back to <testcase name>."""
    p = tmp_path / "report.xml"
    p.write_text(
        '<?xml version="1.0"?><testsuites><testsuite><testcase '
        'classname="tests.playwright.test_login" name="test_from_name_only">'
        '<failure type="X" message="boom">'
        'conftest.py:10: in helper_only\n  call()\n'
        '</failure>'
        '</testcase></testsuite></testsuites>',
        encoding="utf-8",
    )
    fails = parse_junit_xml(p, base=tmp_path)
    assert fails[0].test_function == "test_from_name_only"
...
def test_multi_statement_accepts_prerequisite_then_action() -> None:
    body = "pom.locate('agreement').check()\npom.click_sign_in_with_microsoft()"
